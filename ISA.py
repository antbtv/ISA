...
class VM:
    ...
    def _parse_operand(self, tok: str, pc_for_err: int) -> Operand:
        tok = tok.strip()
        # REG
        if tok.lower().startswith("r") and tok[1:].isdigit():
            r = int(tok[1:])
            if not (0 <= r <= 7):
    ...
    def load_program(self, text: str):
        ...
        for line in lines:
            ...
            if not line:
                continue
            # allow "label: instr" or "label:" alone
            while True:
                if ":" in line:
        ...
        for i, line in enumerate(cleaned):
            ...
            if op in ("HALT", "NOP"):
                ...
                a = Operand("IMM", imm=labels[rest])  # store target as instruction index
            else:
                if not rest:
...
result = vm.run()
print(result)
print("sum =", vm.MEM[N + 1])  # должно быть 114